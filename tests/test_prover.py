...
def test_dummy_type_str():
    assert str(DummyType()) == "Dummy"
...
fn main() -> Int
{
  ...
  while i < 3
    ...
    invariant forall j: Int :: 0 <= j and j < length(a) => a[j] == j
    decreases 3 - i
  {
    push(a, i);
    i = i + 1;
...
"""
    ...
    assert failed
    assert any(
        oc.counterexample and any(e.startswith("a = ") for e in oc.counterexample)
        for oc in failed
    )
