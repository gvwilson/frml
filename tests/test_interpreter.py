...
def test_param_elem_hint_unknown_function():
    ...
    assert vm._param_elem_hint("nope", 0, ast.LiteralInt(1, 0, 0)) is None
...
def test_eval_result_outside_postcondition():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprVar("result", 0, 0))


def test_eval_old_outside_postcondition():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprOld(ast.LiteralInt(1, 0, 0), 0, 0))


def test_eval_old_variable_outside_postcondition():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprVar("x", 0, 0), use_old=True)
...
def test_eval_old_unknown_variable():
    ...
    vm.snapshot = {}
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprVar("missing", 0, 0), use_old=True)


def test_eval_unknown_unary_operator():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprUnary("~", ast.LiteralInt(1, 0, 0), 0, 0))
...
def test_eval_unknown_binary_operator():
    ...
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(
            ast.ExprBinary("~", ast.LiteralInt(1, 0, 0), ast.LiteralInt(1, 0, 0), 0, 0)
        )
...
def test_eval_length_on_non_array():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm.eval_expr(ast.ExprLength(ast.LiteralInt(1, 0, 0), 0, 0))
...
def test_infer_array_elem_empty_without_hint():
    vm = interp()
    with pytest.raises(FrmlRuntimeError):
        vm._infer_array_elem(ast.ExprArrayLiteral([], 0, 0), None, False)
...
def test_infer_array_elem_non_scalar_element():
    ...
    expr = ast.ExprArrayLiteral([ast.ExprVar("a", 0, 0)], 0, 0)
    with pytest.raises(FrmlRuntimeError):
...
def test_match_bound_non_binary_returns_none():
    vm = interp()
    assert vm._match_bound(ast.ExprVar("i", 0, 0), "i") is None


def test_match_bound_left_side_operators():
    vm = interp()
    i = ast.ExprVar("i", 0, 0)
    e = ast.LiteralInt(3, 0, 0)
    assert vm._match_bound(ast.ExprBinary("<", i, e, 0, 0), "i") == ("lt", e)
    assert vm._match_bound(ast.ExprBinary("<=", i, e, 0, 0), "i") == ("le", e)
    assert vm._match_bound(ast.ExprBinary(">=", i, e, 0, 0), "i") == ("low", e)
    assert vm._match_bound(ast.ExprBinary(">", i, e, 0, 0), "i") == ("low_gt", e)
...
def test_match_bound_right_side_operators():
    ...
    i = ast.ExprVar("i", 0, 0)
    e = ast.LiteralInt(3, 0, 0)
    assert vm._match_bound(ast.ExprBinary(">", e, i, 0, 0), "i") == ("lt", e)
    assert vm._match_bound(ast.ExprBinary(">=", e, i, 0, 0), "i") == ("le", e)
    assert vm._match_bound(ast.ExprBinary("<=", e, i, 0, 0), "i") == ("low", e)
    assert vm._match_bound(ast.ExprBinary("<", e, i, 0, 0), "i") == ("low_gt", e)


def test_extract_bounds_low_bound_not_zero_is_kept():
    vm = interp()
    i = ast.ExprVar("i", 0, 0)
    conjuncts = [ast.ExprBinary(">=", i, ast.LiteralInt(5, 0, 0), 0, 0)]
...
def test_extract_bounds_le_upper_bound():
    ...
    length_expr = ast.ExprLength(ast.ExprVar("a", 0, 0), 0, 0)
    conjuncts = [
        ast.ExprBinary(">=", i, ast.LiteralInt(0, 0, 0), 0, 0),
        ast.ExprBinary("<=", i, length_expr, 0, 0),
    ]
    low, high = vm._extract_bounds(conjuncts, "i")
    assert (low, high) == (0, 4)
...
def test_extract_bounds_swallows_eval_failure():
    ...
    vm.push_scope()
    vm.scopes[-1]["x"] = 5
    length_expr = ast.ExprLength(ast.ExprVar("x", 0, 0), 0, 0)
    conjuncts = [ast.ExprBinary("<", i, length_expr, 0, 0)]
...
def test_eval_quantifier_unknown_kind():
    vm = interp()
    q = ast.ExprQuantifier("bogus", "i", INT, ast.LiteralBool(True, 0, 0), 0, 0)
