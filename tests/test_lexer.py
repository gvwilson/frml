...
from frml.lexer import Token, _is_digit, _is_ident_char, _is_ident_start, tokenize
...
def test_identifier_character_helpers():
    assert _is_ident_start("a")
    assert _is_ident_start("Z")
    assert not _is_ident_start("1")
    assert _is_ident_char("_")
    assert _is_ident_char("9")
    assert not _is_ident_char("!")
    assert _is_digit("0")
    assert _is_digit("9")
    assert not _is_digit("a")
