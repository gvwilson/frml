...
from frml.prover_complete import CheckOutcome, Obligation
from frml.utils import Level, Position
...
def test_do_check_loop_accepts_loop(capsys, write_frml):
    path = write_frml(
        "fn main() -> Int { let i: Int = 0;"
        " while i < 3 invariant 0 <= i and i <= 3 decreases 3 - i"
        " { i = i + 1; } return i; }"
    )
    assert do_check(path, level=Level.LOOP) == 0
    assert "VERIFIED" in capsys.readouterr().out


def test_do_check_loop_rejects_array(capsys, write_frml):
    path = write_frml("fn main() -> Int { let a: Array<Int> = [1]; return length(a); }")
    assert do_check(path, level=Level.LOOP) == 1
    err = capsys.readouterr().err
    assert "TypeError" in err
    assert "array variable 'a' is not available at level 'loop'" in err


def test_do_check_loop_rejects_builtin(capsys, write_frml):
    path = write_frml('fn main() -> Int { print("x"); return 0; }')
    assert do_check(path, level=Level.LOOP) == 1
    err = capsys.readouterr().err
    assert "TypeError" in err
    assert "built-in function 'print' is not available at level 'loop'" in err


def test_do_check_array_accepts_array_program(capsys, write_frml):
    path = write_frml(
        "fn main() -> Int {"
        " let a: Array<Int> = [1, 2];"
        " let i: Int = 0;"
    ...
    assert do_check(path, level=Level.ARRAY) == 0
    assert "VERIFIED" in capsys.readouterr().out


def test_do_check_array_accepts_array_returning_function(capsys, write_frml):
    path = write_frml(
        "fn f() -> Array<Int> ensures length(result) == 2 { return [1, 2]; }"
        " fn main() -> Int { let a: Array<Int> = f(); return a[1] - 2; }"
    )
    assert do_check(path, level=Level.ARRAY) == 0
    assert "VERIFIED" in capsys.readouterr().out


def test_do_check_array_rejects_builtin(capsys, write_frml):
    path = write_frml('fn main() -> Int { print("x"); return 0; }')
    assert do_check(path, level=Level.ARRAY) == 1
    err = capsys.readouterr().err
    assert "TypeError" in err
    assert "built-in function 'print' is not available at level 'array'" in err


def test_do_check_builtin_accepts_builtin_program(capsys, write_frml):
    path = write_frml(
        "fn main() -> Int {"
        " let a: Array<Int> = [1, 2];"
        " push(a, 3);"
    ...
    assert do_check(path, level=Level.BUILTIN) == 0
    assert "VERIFIED" in capsys.readouterr().out


def test_do_check_builtin_accepts_io(capsys, write_frml):
    path = write_frml(
        'fn main() -> Int { let s: String = read("x"); write("x", s); return 0;}'
    )
    assert do_check(path, level=Level.BUILTIN) == 0
    assert "VERIFIED" in capsys.readouterr().out
